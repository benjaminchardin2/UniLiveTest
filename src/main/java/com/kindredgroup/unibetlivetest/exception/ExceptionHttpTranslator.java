...
public class ExceptionHttpTranslator {
    ...
    @ExceptionHandler(CustomNonStandardException.class)
    public ResponseEntity businessNonStandardException(HttpServletRequest request, final CustomNonStandardException e) {
        return ResponseEntity
                .status(e.getException().getCode())
                .body(new ExceptionDto().setErrormessage(e.getMessage()).setPath(request.getServletPath()));
    }
